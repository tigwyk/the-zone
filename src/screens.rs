...
use crate::area::{ItemKind, VendorStock, ZoneData, MESSAGE_ROW};
...
pub(crate) fn draw_chrome(grid: &mut TileGrid, run: &RunState) {
    let status = format!(
        "  HP {}/{}  RAD {}  RU {}  Day {} {}",
        run.hp,
        run.max_hp,
...
pub(crate) fn build_inventory_grid(
    ...
    for (i, name) in ATTR_NAMES.iter().enumerate() {
        grid.text(44, LIST_ROW + i, &format!("{name} {}", run.attrs[i]), PALETTE.status, false);
    }
    for (i, name) in SKILL_NAMES.iter().enumerate() {
...
pub(crate) fn build_trade_grid(
    ...
    for (i, (id, n)) in list.iter().enumerate() {
        let item = &zone.items[id];
        let p = price(item.base, vendor.markup, run.skills[BARTER], rep, buying);
...
pub(crate) fn trade_one(
    ...
    let item = &zone.items[&id];
    let Some(p) = price(item.base, vendor.markup, run.skills[BARTER], rep, buying) else {
...
mod tests {
    ...
    fn every_screen_builds() {
        ...
        for id in zone.areas.keys() {
            build_area_grid(&mut grid, &zone, &run, id, 0, "test");
        }
        build_inventory_grid(&mut grid, &zone, &run, 0, "test");
        build_trade_grid(&mut grid, &zone, &stock, &run, "trader", true, 0, "test");
        build_trade_grid(&mut grid, &zone, &stock, &run, "trader", false, 0, "test");
